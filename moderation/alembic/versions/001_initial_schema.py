...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_product_snapshots_is_moderated'), 'product_snapshots', ['is_moderated'])
    ...
    op.create_table(
        ...
        sa.Column('decision', sa.Enum('APPROVED', 'DECLINED', name='moderation_decision_type'), nullable=False),
        ...
        sa.ForeignKeyConstraint(['reason_id'], ['blocking_reasons.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_moderation_decisions_snapshot_id'), 'moderation_decisions', ['snapshot_id'])
    op.create_index(op.f('ix_moderation_decisions_moderator_id'), 'moderation_decisions', ['moderator_id'])
